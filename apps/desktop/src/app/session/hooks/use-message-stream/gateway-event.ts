...
import { normalizeMessageActions, routeMessageActions } from '@/lib/message-actions'
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        const actions = normalizeMessageActions(payload?.actions)

        if (actions.length) {
          updateSessionState(sessionId, state => {
            const routed = routeMessageActions(state.messages, actions)

            return {
              ...state,
              messages: routed.messages,
              pendingMessageActions: routed.pendingMessageActions
            }
          })
        }
