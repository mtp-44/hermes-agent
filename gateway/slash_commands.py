    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        if _cache_lock is not None:
            ...
            if _old_agent is not None:
                # Capture the conversation to durable memory (Open Brain)
                # before tearing the agent down — /new and /reset are
                # session boundaries, so flush pending extraction now.
                try:
                ...
                # Offload the (synchronous, potentially long) teardown to a
                # worker thread with a bounded timeout so a wedged cleanup can't
                # block the event loop and silence the bot (#35994).
                try:
