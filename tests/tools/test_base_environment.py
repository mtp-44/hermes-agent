...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_template_is_quoted(self):
        """The mktemp template must be shlex-quoted as one word (Windows/Git-Bash
        ``C:/Users/...`` or spaces), and the allocated path is always expanded
        from a double-quoted shell variable. (Fork adaptation of the former
        $BASHPID quoting test after porting upstream 911d380296.)"""
        env = _TestableEnv()
        env._snapshot_ready = True
        env._snapshot_path = "/tmp/has space/hermes-snap-x.sh"
        wrapped = env._wrap_command("echo hi", "/tmp")
        assert "mktemp '/tmp/has space/hermes-snap-x.sh.tmp.XXXXXXXXXX'" in wrapped
        assert " space/hermes-snap-x.sh.tmp.XXXXXXXXXX" not in wrapped.replace(
            "'/tmp/has space/hermes-snap-x.sh.tmp.XXXXXXXXXX'", "")
        assert 'mv -f "$__hermes_snap_tmp"' in wrapped
        assert "$BASHPID" not in wrapped
