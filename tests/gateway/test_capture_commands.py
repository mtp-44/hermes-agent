...
async def test_claude_route_switches_session_override(mock_resolve_runtime, mock_load_gateway_config):
    ...
    result = await runner._handle_model_route_command(_make_event("/claude"), "claude")

    assert "Claude Sonnet" in result
    session_key = build_session_key(_make_source())
    assert runner._session_model_overrides[session_key]["route"] == "claude"
...
@patch("gateway.run._load_gateway_config", return_value={})
@patch("hermes_cli.runtime_provider.resolve_runtime_provider")
async def test_opus_route_switches_session_override(mock_resolve_runtime, mock_load_gateway_config):
    runner = _make_runner()
    mock_resolve_runtime.return_value = {
        "provider": "openrouter",
        "api_key": "sk-or-test",
    ...
    result = await runner._handle_model_route_command(_make_event("/opus"), "opus")
