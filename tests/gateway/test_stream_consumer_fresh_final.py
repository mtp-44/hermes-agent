...
# The consumer ages previews with time.monotonic(), whose zero is roughly host
# boot. "Created at 0.0" is only stale once the host has been up longer than
# the threshold, which a freshly booted CI runner often hasn't. Pin an age
# well past every threshold in this file (max 120 s) instead.
def _stale_created_ts() -> float:
...
class TestFreshFinalForLongLivedPreviews:
    ...
    async def test_disabled_by_default_still_edits_in_place(self):
        ...
        consumer._message_created_ts = _stale_created_ts()
