...
# -- Fork-local call-sites (our tree predates upstream's working_diff /
# subagent_worktree / goals-fingerprint modules; these are the equivalent
# automatic or workspace-driven git paths that exist here). ----------------


def test_gateway_repo_file_listing_is_safe(malicious_repo):
    from tui_gateway import server
    repo, marker = malicious_repo
    server._fuzzy_cache.clear()
    server._list_repo_files(str(repo))
    assert _fired(marker) == []


def test_web_server_branch_probe_is_safe(malicious_repo):
    from hermes_cli import web_server
    repo, marker = malicious_repo
    web_server._fs_git_branch(str(repo))
    assert _fired(marker) == []


def test_cli_worktree_setup_is_safe(malicious_repo):
    import cli as cli_mod
    repo, marker = malicious_repo
    info = cli_mod._setup_worktree(str(repo), sync_base=False)
    assert info is not None, "worktree add should still succeed under hardening"
    assert _fired(marker) == []
    assert cli_mod._worktree_has_unpushed_commits(info["path"]) in (True, False)
    assert _fired(marker) == []


def test_kanban_worktree_add_is_safe(malicious_repo, tmp_path):
    from hermes_cli import kanban_db
    repo, marker = malicious_repo
    assert kanban_db._git_toplevel(repo) is not None
    kanban_db._ensure_git_worktree(repo, tmp_path / "kanban-wt", "safe-kanban")
    assert (tmp_path / "kanban-wt" / "README").exists()
    assert _fired(marker) == []


def test_context_reference_log_patch_is_safe(malicious_repo):
    from agent import context_references as cr
    repo, marker = malicious_repo
    ref = type("R", (), {"raw": "@git:1"})()
    cr._expand_git_reference(ref, repo, ["log", "-1", "-p"], "git log -1 -p")
    assert _fired(marker) == []
