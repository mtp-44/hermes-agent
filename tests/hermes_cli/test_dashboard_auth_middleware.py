...
def test_gated_pwa_static_files_are_public(gated_app, path):
    """PWA installability files and release metadata must bypass the gate.
    ...
    (manifest/sw.js/release metadata are static; the icons are
    unauthenticated-safe by the same threshold as /favicon.ico), so 404
    (route reached, file missing in the test app's dist) is fine — 401 or a
    /login redirect is not.
    """
    r = gated_app.get(path, follow_redirects=False)
